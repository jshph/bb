...
The `sidebarProgressiveDisclosure` experiment is on by default. In **By
project** and **By machine**, it keeps attention groups visible, shows the five
root trees with the most recently finished descendant, and reveals ten more per
**Show more** click. Parent and child rows stay together, and revealed groups
remain visible through activity and sort-order changes. **Manually** is
unchanged. Toggle it with `bb settings experiment
